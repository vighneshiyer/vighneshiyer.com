...
Dean says we have been doing this for long time and we are just extending the scope of them. Shankar is saying that the entire product dev process is the entire virtual simulation, all encompassingPaul, more overlap as we move from package to system, need to do more codesign for opt, need better physical scaling due to abstractions overlapShankar, first build chip then write software, but new way is to write software directly as the chip is being developed, digital twin in action, software has to know how the chip interacts with the product and environment
